...
exports.ServerAdaptiveAdversaryAuthority = void 0;
const admin = __importStar(require("firebase-admin"));
const crypto = __importStar(require("crypto"));
const https_1 = require("firebase-functions/v2/https");
class ServerAdaptiveAdversaryAuthority {
    db;
    constructor(db) {
     ...
     */
    async selectAuthoritativeTargetFailureMode(authenticatedUid) {
     ...
     */
    buildAdaptivePolicy(patternType, observationCount) {
        const tier = observationCount >= 3 ? 'HIGH_CONFIDENCE' : observationCount === 2 ? 'REPEATED' : 'OBSERVED';
        switch (patternType) {
            case 'PREMATURE_ESCALATION':
                return {
                    targetFailureMode: patternType,
                    challengeTier: tier,
                    policyObjective: tier === 'HIGH_CONFIDENCE'
     ...
     */
    async generateAuthoritativeAdaptiveChallenge(authenticatedUid) {
        if (!authenticatedUid || typeof authenticatedUid !== 'string') {
            throw new https_1.HttpsError('unauthenticated', 'Authenticated UID is required.');
        }
        ...
        const patternType = targetPattern ? targetPattern.patternType : 'INSUFFICIENT_CORRELATION';
     ...
     */
    buildScenarioTemplate(policy, challengeId) {
     ...
     */
    async evaluateAdaptiveChallengeSubmission(authenticatedUid, req) {
        ...
        if (!req || !req.challengeId || !req.selectedActionId) {
            throw new https_1.HttpsError('invalid-argument', 'challengeId and selectedActionId are required.');
        }
        const challengeRef = this.db.doc(`learners/${authenticatedUid}/adaptive_challenges/${req.challengeId}`);
        const challengeSnap = await challengeRef.get();
        if (!challengeSnap.exists) {
            throw new https_1.HttpsError('not-found', `Adaptive challenge '${req.challengeId}' not found.`);
        }
        const state = challengeSnap.data();
        if (state.ownerAuthUid !== authenticatedUid) {
            throw new https_1.HttpsError('permission-denied', `Learner isolation violation: cannot evaluate another learner's challenge.`);
        }
        ...
        for (const evi of submittedEvidence) {
            if (!validEvidenceIds.includes(evi)) {
                throw new https_1.HttpsError('invalid-argument', `Foreign evidence '${evi}' does not belong to challenge evidence pool.`);
            }
        }
...
exports.ServerAdaptiveAdversaryAuthority = ServerAdaptiveAdversaryAuthority;
//# sourceMappingURL=adaptiveAdversaryAuthority.js.map