// web/src/NeuralGraphMeshEngine.ts
/**
 * ============================================================================
...
export interface NeuralVertex {
  ...
  centralityScore: number; // 0.0 to 1.0 (PageRank centrality normalized)
...
export class NeuralGraphMeshEngineService {
  ...
  // ==========================================================================
  // DYNAMIC TOPOLOGY MUTATIONS (ADD / UPDATE / REMOVE)
  // ==========================================================================
  public addVertex(vertex: Partial<NeuralVertex> & { id: string; label: string }): NeuralVertex {
    const existing = this.vertices.get(vertex.id);
    const newVertex: NeuralVertex = {
      id: vertex.id,
      label: vertex.label,
      type: vertex.type || (existing?.type ?? 'IP'),
      category: vertex.category || (existing?.category ?? 'BENIGN_INTERNAL'),
      threatWeight: vertex.threatWeight ?? (existing?.threatWeight ?? 0.1),
      centralityScore: existing?.centralityScore ?? 0.0,
      reputationScore: vertex.reputationScore ?? (existing?.reputationScore ?? 10),
      inDegree: existing?.inDegree ?? 0,
      outDegree: existing?.outDegree ?? 0,
      weightedInDegree: existing?.weightedInDegree ?? 0,
      weightedOutDegree: existing?.weightedOutDegree ?? 0,
      isC2Node: vertex.category === 'C2_CONTROLLER' || (existing?.isC2Node ?? false),
      isAnomalousCluster: existing?.isAnomalousCluster ?? false,
      clusteringCoefficient: existing?.clusteringCoefficient ?? 0,
      lastObserved: new Date().toISOString(),
      associatedIncidents: vertex.associatedIncidents || existing?.associatedIncidents || [],
      geoMetadata: vertex.geoMetadata || existing?.geoMetadata
    };
    this.vertices.set(vertex.id, newVertex);
    this.runPageRank();
    return this.vertices.get(vertex.id)!;
  }

  public addEdge(edge: Partial<NeuralEdge> & { id: string; source: string; target: string }): NeuralEdge {
    const newEdge: NeuralEdge = {
      id: edge.id,
      source: edge.source,
      target: edge.target,
      weight: edge.weight ?? 1.0,
      vectorType: edge.vectorType ?? 'BEACON',
      bytesTransferred: edge.bytesTransferred ?? 1024,
      packetCount: edge.packetCount ?? 1,
      anomalousEntropy: edge.anomalousEntropy ?? 0.5,
  ...
  public removeVertex(vertexId: string): boolean {
